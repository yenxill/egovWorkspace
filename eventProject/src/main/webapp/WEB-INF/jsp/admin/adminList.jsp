		    ...
		    success: function (data, status, xhr) {
		    	...
		    	if(data.list.length >0){
		    		for(var i=0; i<data.list.length; i++){
		    			adminHtml += '<tr>';
		    			adminHtml += '<td>';
		    			adminHtml += data.list[i].rnum;
		    			adminHtml += '</td>';
		    			adminHtml += '<td>';
		    			adminHtml += '<a href="javascript:fn_detail(\''+data.list[i].boardIdx+'\');">';
		    			adminHtml += data.list[i].boardTitle;
		    			adminHtml += '</a>';
		    			adminHtml += '</td>';
		    			adminHtml += '<td>';
		    			adminHtml += data.list[i].createId;
		    			adminHtml += '</td>';
		    			adminHtml += '<td>';
		    			adminHtml += data.list[i].createDate;
		    			adminHtml += '</td>';
		    			adminHtml += '</tr>';
		    		}
		    	}else{
		    		adminHtml += '<tr>';
		    		adminHtml += '<td colspan="4" style="text-align:center;">조회된 결과가 없습니다.</td>';
		    		adminHtml += '</tr>';
		    	}
		    	
		    	$("#tbody").html(adminHtml);
